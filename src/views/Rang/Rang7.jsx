...
function Rang7({ user, heroes }) {
  const sortedHeroes = heroes.sort((a, b) => {
    const winPercentageA = (a["7_win"] / a["7_pick"]) * 100;
    const winPercentageB = (b["7_win"] / b["7_pick"]) * 100;
          ...
          <tbody>
            {topHeroes.map((hero) => (
              <tr key={hero.id} className="hero-row">
                ...
                <td className="hero-stats">{hero["7_pick"]}</td>
                <td className="hero-stats">{hero["7_win"]}</td>
                <td className="hero-stats">
                  {((hero["7_win"] / hero["7_pick"]) * 100).toFixed(2)}%
                </td>
              </tr>
            ))}
          </tbody>
